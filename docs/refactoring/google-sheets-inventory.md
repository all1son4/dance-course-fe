...
| Telegram access tokens                   | `R/W/M/C`               | [`telegram/access.ts`](../../src/lib/telegram/access.ts), maintenance                                                                                                                                                                        | Telegram access repository with atomic issue, claim, revoke, and hash lookup commands           | Runtime reads removed in `DROP-01`; remove legacy write/claim next                                     |
| Telegram user bindings                   | `W/M/C`                 | [`telegram/access.ts`](../../src/lib/telegram/access.ts), maintenance                                                                                                                                                                        | Binding and entitlement repositories with explicit identity rules                               | Runtime reads removed in `DROP-01`; remove the legacy mirror after PostgreSQL write/claim verification |
...
| [`telegram/access`](../../src/lib/telegram/access.ts)                                                             | PostgreSQL reads plus mode-aware writes through the isolated persistence boundary                   | `W/C/T`         | Finish legacy write/claim removal in `DROP-01` |
| [`telegram/access-persistence`](../../src/lib/telegram/access-persistence.ts)                                     | isolated legacy payment/token/binding writes and token claim, Google errors, and compatibility DTOs | `W/M/C/E/T`     | Finish legacy write/claim removal in `DROP-01` |
...
2. Legacy Telegram token claims and payment/token/binding writes still use the
   persistence boundary; all Telegram reads now use PostgreSQL unconditionally.
...
and collection semantics, and fail-closed database errors. Its write/claim boundary
is unchanged in this slice. Compatibility record types remain until `DROP-04`.
