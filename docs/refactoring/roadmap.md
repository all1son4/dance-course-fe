...
Status: `IN_PROGRESS`. The implementation now routes explicit database mode through
the atomic invoice allocator and purpose-specific report/campaign repositories.
...
Local type, format, lint, and 69 unit tests pass. PostgreSQL 17 migration/concurrency
evidence, remote Quality, the development migration, invariant audit, and deployed
development smoke remain required before `DONE`.
...
| 2026-08-13 | WRITE-06 implementation     | `IN_PROGRESS` | Local checks pass; PG17 and deployed evidence pending         |
