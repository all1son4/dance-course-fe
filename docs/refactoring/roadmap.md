...
Status: `IN_PROGRESS`. Timed channel access and legacy bot-start access now use an
...
reused by ordinary purchases. Enabling the flag remains a `CUT-03` operation. Final
status awaits exact-revision Quality and deployed development smoke evidence.
...
| 2026-08-13 | WRITE-04 implementation     | `IN_PROGRESS` | DB-only timed/legacy access path; local acceptance passes     |
